...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{
      ...
      <span style={{ width: 5, height: 5, borderRadius: '50%', background: si.dot, flexShrink: 0, display: 'inline-block' }} />
      {si.label}
    </span>
  )
}
      ...
      {budget > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#9ca3af', marginBottom: 4, fontFamily: 'monospace' }}>
            ...
            <span style={{ color: budgetColor, fontWeight: 600 }}>{logged.toFixed(1)}h / {budget}h</span>
          </div>
          <div style={{ height: 5, background: '#eaecef', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${budgetPct}%`, background: budgetColor, borderRadius: 3, transition: 'width 0.4s ease' }} />
          </div>
        </div>
      )}

      {/* Footer meta */}
      <div style={{ display: 'flex', gap: 20, marginTop: 12, borderTop: '1px solid #f3f4f6', paddingTop: 12 }}>
        {project.due_date && (
          <div>
...
export default function ProjectsPage() {
  ...
  async function loadProjects() {
    ...
    const { data: clientData } = await supabase
      ...
      .eq('workspace_id', member.workspace_id).order('name')
  ...
  return (
    <AppLayout>
      <div style={{
        ...
        <span style={{ fontWeight: 700, fontSize: 15, color: '#1a1a2e' }}>Projects</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px' }}>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', padding: '60px 24px', textAlign: 'center' }}>
            ...
            {!search && <button onClick={() => setShowModal(true)} style={primaryBtnStyle}>+ New Project</button>}
          </div>
        )}
